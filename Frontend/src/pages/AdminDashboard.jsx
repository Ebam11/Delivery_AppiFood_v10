...
const generateMockSeries = ({ points = 12, base = 100, wave = 24, trend = 4, floor = 8 }) => (
  Array.from({ length: points }, (_, i) => {
    const val = base + Math.sin((i + 1) * 0.9) * wave + i * trend + ((i % 3) - 1) * 6
...
function DashboardSection({ restaurants, users, orders, t }) {
  const estimatedMonthlyOrdersBase = Math.max(orders.length * 20, 80)
  const BARS   = generateMockSeries({ points: 12, base: estimatedMonthlyOrdersBase, wave: 34, trend: 7, floor: 45 })
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[560px] flex items-end gap-1.5 h-44 pr-2">
              {BARS.map((v, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-lg" style={{ height: toPercent(v, maxBar), background: i === 11 ? '#1a202c' : P }} />
                  <span className="text-gray-400" style={{ fontSize:'10px' }}>{MONTHS[i]}</span>
                </div>
              ))}
            </div>
          </div>
...
function ReportsSection({ restaurants, users, orders, t }) {
  const MONTHS       = [t('adminDashboard.months.jan'),t('adminDashboard.months.feb'),t('adminDashboard.months.mar'),t('adminDashboard.months.apr'),t('adminDashboard.months.may'),t('adminDashboard.months.jun'),t('adminDashboard.months.jul'),t('adminDashboard.months.aug'),t('adminDashboard.months.sep'),t('adminDashboard.months.oct'),t('adminDashboard.months.nov'),t('adminDashboard.months.dec')]
  const estimatedRevenueBase = Math.max(Math.round((orders.reduce((s, o) => s + o.amount, 0) * 3.6) / 12), 75)
  const estimatedUsersBase = Math.max(Math.round((users.length * 18) / 12), 25)
  const estimatedOrdersBase = Math.max(Math.round((orders.length * 35) / 12), 55)
  const REVENUE_DATA = generateMockSeries({ points: 12, base: estimatedRevenueBase, wave: 22, trend: 5, floor: 35 })
  const USERS_DATA   = generateMockSeries({ points: 12, base: estimatedUsersBase, wave: 14, trend: 3, floor: 18 })
  const ORDERS_DATA  = generateMockSeries({ points: 12, base: estimatedOrdersBase, wave: 28, trend: 6, floor: 30 })
  const maxRev   = Math.max(...REVENUE_DATA, 1)
  const maxUsers = Math.max(...USERS_DATA, 1)
  const maxOrds  = Math.max(...ORDERS_DATA, 1)
  const topRest  = [...restaurants].sort((a,b) => b.orders - a.orders).slice(0,5)
  const maxOrders = topRest[0]?.orders || 1
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[560px] flex items-end gap-1.5 h-40 pr-2">
              {REVENUE_DATA.map((v,i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-lg" style={{ height: toPercent(v, maxRev), background: P }} />
                  <span className="text-gray-400" style={{ fontSize:'10px' }}>{MONTHS[i]}</span>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[560px] flex items-end gap-1.5 h-40 pr-2">
              {USERS_DATA.map((v,i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-lg" style={{ height: toPercent(v, maxUsers), background:'#1a202c' }} />
                  <span className="text-gray-400" style={{ fontSize:'10px' }}>{MONTHS[i]}</span>
                </div>
              ))}
            </div>
                ...
                <div key={r.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                      <div className="h-full rounded-full" style={{ width: toPercent(r.orders, maxOrders), background: P }} />
                    </div>
                  </div>
                </div>
              )
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm">
          ...
          <div className="overflow-x-auto">
            <div className="min-w-[560px] flex items-end gap-1.5 h-40 pr-2">
              {ORDERS_DATA.map((v,i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div className="w-full rounded-t-lg" style={{ height: toPercent(v, maxOrds), background:'#fbbf24' }} />
                  <span className="text-gray-400" style={{ fontSize:'10px' }}>{MONTHS[i]}</span>
                </div>
              ))}
            </div>
