          ...
          <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
            {supportShortcuts.map((item) => (
              ...
              >
                ...
                <h2 className="font-black text-gray-900 text-lg">{t(`supportPage.shortcuts.${item.label.replace(/ /g, '_')}`, { defaultValue: item.label })}</h2>
                <p className="mt-1 text-sm text-gray-500">{t('supportPage.shortcut_desc') || 'Acceso directo para resolver un caso frecuente.'}</p>
              </Link>
            ))}
          </div>
        </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr] items-start">
          <div className="bg-white rounded-3xl border border-gray-100 shadow-xl overflow-hidden">
            <div className="p-6 border-b border-gray-100">
              <h2 className="text-2xl font-black text-gray-900">{t('supportPage.assistant_title') || 'Asistente AppiFood'}</h2>
              <p className="text-gray-500 mt-1">{t('supportPage.assistant_desc') || 'Respuestas inmediatas para soporte y navegación dentro de la app.'}</p>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-3xl border border-gray-100 shadow-lg p-6">
              <h2 className="text-2xl font-black text-gray-900 mb-4">{t('supportPage.faqs_title') || 'Preguntas frecuentes'}</h2>
              <div className="space-y-6">
                  ...
                  <div key={group.category}>
                    <div className="flex items-center gap-3 mb-3">
                      <span className={`inline-flex px-3 py-1.5 rounded-full text-xs font-bold border ${faqBadgeStyles[group.category] || 'bg-gray-50 text-gray-700 border-gray-100'}`}>
                        {t(`supportPage.faq_category.${group.category.replace(/ /g, '_')}`, { defaultValue: group.category })}
                      </span>
                    </div>
                    <div className="space-y-4">
                      {group.items.map((faq) => (
                        <div key={faq.question} className="p-4 rounded-2xl bg-gray-50 border border-gray-100">
                          <h3 className="font-bold text-gray-900">{t(`supportPage.faq_q.${faq.question.slice(0, 15).replace(/ /g, '_')}`, { defaultValue: faq.question })}</h3>
                          <p className="mt-2 text-sm text-gray-600 leading-relaxed">{t(`supportPage.faq_a.${faq.question.slice(0, 15).replace(/ /g, '_')}`, { defaultValue: faq.answer })}</p>
                        </div>
                      ))}
                    </div>
                  </div>
