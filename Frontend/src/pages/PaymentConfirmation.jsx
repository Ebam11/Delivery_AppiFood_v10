// Archivo: src/pages/PaymentConfirmation.jsx
        ...
        {status === 'success' && (
          <div className="space-y-4">
            <div className="bg-gradient-to-br from-emerald-500 to-teal-600 rounded-2xl shadow-xl shadow-emerald-500/20 p-6 text-center text-white">
              ...
              <h1 className="text-2xl font-black mb-1">{t('payment_confirmation.success_title')}</h1>
              ...
              <div className="mt-3 inline-flex items-center gap-2 bg-white/15 rounded-full px-4 py-1.5 text-xs font-bold">
                <i className="fas fa-lock" /> {t('payment_confirmation.secure_transaction') || 'Transacción segura verificada'}
              </div>
            </div>

            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-100 dark:border-slate-700 overflow-hidden">
              <div className="bg-gray-50 dark:bg-slate-700/50 px-5 py-3 flex items-center justify-between">
                <span className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider">{t('payment_confirmation.order_label') || 'Pedido'}</span>
              ...
              <div className="p-5 space-y-4">
                {order?.restaurant_name && (
                  <div className="flex items-center gap-3 pb-3 border-b border-gray-100 dark:border-slate-700">
                    ...
                    <div>
                      <p className="text-xs text-gray-500 dark:text-gray-400 font-semibold">{t('payment_confirmation.restaurant_label') || 'Restaurante'}</p>
                ...
                {order?.items && order.items.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-xs font-black text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">{t('payment_confirmation.products_label') || 'Productos'}</p>
                ...
                <div className="border-t border-gray-100 dark:border-slate-700 pt-3 space-y-1.5 text-sm">
                  <div className="flex justify-between text-gray-500 dark:text-gray-400">
                    <span>{t('payment_confirmation.subtotal_label') || 'Subtotal'}</span>
                    <span className="font-semibold">{fmt(subtotal)}</span>
                  </div>
                  <div className="flex justify-between text-gray-500 dark:text-gray-400">
                    <span>{t('payment_confirmation.shipping_label') || 'Envío'}</span>
                    <span className="font-semibold">{fmt(deliveryCost)}</span>
                  </div>
                  {discount > 0 && (
                    <div className="flex justify-between text-emerald-600 font-semibold">
                      <span>{t('payment_confirmation.discount_label') || 'Descuento'}</span>
                  ...
                  <div className="flex justify-between font-black text-base text-gray-800 dark:text-white border-t border-gray-200 dark:border-slate-600 pt-2 mt-2">
                    <span>{t('payment_confirmation.total_paid_label') || 'Total pagado'}</span>
                ...
                {order?.delivery_address && (
                  <div className="bg-gray-50 dark:bg-slate-700/50 rounded-xl p-3 flex items-start gap-3 text-sm">
                    ...
                    <div>
                      <p className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase mb-0.5">{t('payment_confirmation.delivery_address_label') || 'Entrega en'}</p>
            ...
            <div className="space-y-3">
              ...
              >
                <i className="fas fa-box-open" /> {t('payment_confirmation.view_order')}
              </button>
              <button
                onClick={() => navigate('/')}
              ...
              >
                <i className="fas fa-home" /> {t('payment_confirmation.continue_shopping')}
              </button>
            </div>
            ...
            <div className="flex items-center justify-center gap-4 text-xs text-gray-400 dark:text-gray-500 font-semibold pt-2">
              ...
              <span className="flex items-center gap-1.5"><i className="fas fa-shield-alt text-blue-500" /> {t('payment_confirmation.encrypted_data') || 'Datos Cifrados'}</span>
        ...
        {status === 'pending' && (
          <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-md p-8 text-center border border-gray-100 dark:border-slate-700">
            ...
            <button onClick={() => navigate('/')} className="w-full text-gray-500 dark:text-gray-400 font-bold py-2 text-sm hover:text-[#FF4B3E] transition">
              {t('payment_confirmation.back_to_home') || 'Volver al inicio'}
            </button>
          </div>
        )}
