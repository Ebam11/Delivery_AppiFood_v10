  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: t('adminDashboard.reviews.total', { defaultValue: 'Total' }),
            value: stats.total,
            color: 'text-gray-800 dark:text-slate-100',
          },
          {
            label: t('adminDashboard.reviews.visible', { defaultValue: 'Visibles' }),
            value: stats.visible,
            color: 'text-green-500',
          },
          {
            label: t('adminDashboard.reviews.hidden', { defaultValue: 'Ocultas' }),
            value: stats.hidden,
            color: 'text-orange-500',
          },
          {
            label: t('adminDashboard.reviews.avgRating', { defaultValue: 'Calificación Promedio' }),
          ...
          <div
            key={i}
            className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-gray-100 dark:border-slate-800 shadow-sm text-center"
          >
            <p className={`text-3xl font-black ${stat.color}`}>{stat.value}</p>
            <p className="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase mt-1">
              {stat.label}
            </p>
          </div>
        ))}
              ...
              {visible.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="font-medium text-gray-800 dark:text-slate-200">
                          {r.restaurant?.name || 'Restaurante eliminado'}
                        </p>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <Badge status={isVisible ? 'active' : 'inactive'} />
                    </td>
                    <td className="px-6 py-4">
