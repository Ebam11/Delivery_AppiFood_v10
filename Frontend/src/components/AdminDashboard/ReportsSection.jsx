...
export default function ReportsSection() {
  ...
  const MONTHS = [
    t('adminDashboard.months.jan', { defaultValue: 'Ene' }),
    t('adminDashboard.months.feb', { defaultValue: 'Feb' }),
    t('adminDashboard.months.mar', { defaultValue: 'Mar' }),
    t('adminDashboard.months.apr', { defaultValue: 'Abr' }),
    t('adminDashboard.months.may', { defaultValue: 'May' }),
    t('adminDashboard.months.jun', { defaultValue: 'Jun' }),
    t('adminDashboard.months.jul', { defaultValue: 'Jul' })
  ];
  const REVENUE_DATA = [45, 52, 48, 61, 59, 75, 82];
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={i} className="bg-white dark:bg-slate-900 rounded-2xl p-5 border border-gray-100 dark:border-slate-800 shadow-sm text-center">
            <p className="text-3xl font-black text-gray-800 dark:text-slate-100">{kpi.value}</p>
            <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase mt-1">{kpi.label}</p>
            <span className="inline-block mt-2 px-2 py-1 bg-green-50 dark:bg-green-900/20 text-green-600 dark:text-green-400 rounded-full text-[10px] font-bold">
              {kpi.trend} {t('adminDashboard.reports.vsLastYear', { defaultValue: 'vs año anterior' })}
            </span>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-6 shadow-sm">
          <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-6">
            {t('adminDashboard.reports.monthlyRevenue', { defaultValue: 'Ingresos Mensuales (K$)' })}
          </h3>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-6 shadow-sm">
          <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-6">
            {t('adminDashboard.reports.userGrowth', { defaultValue: 'Crecimiento de Usuarios' })}
          </h3>
          <div className="flex items-end gap-2 h-48 w-full">
            {USERS_DATA.map((val, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2 group">
                <div className="w-full flex flex-col justify-end relative h-full">
                  <div className="w-full bg-slate-700 dark:bg-slate-500 rounded-t-lg transition-all duration-500 group-hover:bg-slate-600 dark:group-hover:bg-slate-400" style={{ height: toPercent(val, 500) }} />
                </div>
                <span className="text-xs text-gray-400 dark:text-slate-500 font-medium">{MONTHS[i]}</span>
              </div>
            ))}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 p-6 shadow-sm">
        <h3 className="font-bold text-gray-800 dark:text-slate-100 mb-4">
          {t('adminDashboard.reports.topRestaurants', { defaultValue: 'Top Restaurantes por Pedidos' })}
        </h3>
          ...
          {[
            ...
            <div key={i}>
              <div className="flex justify-between text-sm mb-1">
                <span className="font-bold text-gray-700 dark:text-slate-300">{i+1}. {r.name}</span>
                <span className="text-gray-500 dark:text-slate-400">{r.orders} {t('adminDashboard.reports.ordersLabel', { defaultValue: 'pedidos' })}</span>
              </div>
